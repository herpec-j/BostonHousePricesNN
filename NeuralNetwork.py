...
class Layer:
    ...
    def __init__(self, inbound_layers=[]):
        ...
        # Keys are the inputs to this layer and their values are
        #  the partials of this layer with respect to that input.
        self.gradients = {}
...
class Input(Layer):
    ...
    def backward(self):
        ...
        for l in self.outbound_layers:
            self.gradients[self] += l.gradients[self]
...
class Linear(Layer):
    ...
    def backward(self):
        ...
        self.gradients = {l: np.zeros_like(l.value) for l in self.inbound_layers}
        # Cycle through the outputs. The gradient will change depending
        # on each output, so the gradients are summed over all outputs.
        for l in self.outbound_layers:
            # Get the partial of the cost with respect to this layer.
            gradient_cost = l.gradients[self]
            # Set the partial of the loss with respect to this layer's inputs.
            self.gradients[self.inbound_layers[0]] += np.dot(gradient_cost, self.inbound_layers[1].value.T)
            # Set the partial of the loss with respect to this layer's weights.
            self.gradients[self.inbound_layers[1]] += np.dot(self.inbound_layers[0].value.T, gradient_cost)
            # Set the partial of the loss with respect to this layer's bias.
            self.gradients[self.inbound_layers[2]] += np.sum(gradient_cost, axis=0, keepdims=False)
...
class Sigmoid(Layer):
    ...
    def backward(self):
        ...
        for l in self.outbound_layers:
            gradient_cost = l.gradients[self]
            sigmoid = self.value
            self.gradients[self.inbound_layers[0]] += sigmoid * (1 - sigmoid) * gradient_cost
...
class MSE(Layer):
    ...
    def forward(self):
		...
		# Save the number of input features. If will be useful for the backward propagation step.
...
def topological_sort(feed_dict):
    ...
    """
    input_layers = [l for l in feed_dict.keys()]
    G = {}
    layers = [l for l in input_layers]
    while len(layers) > 0:
        l = layers.pop(0)
        if l not in G:
            G[l] = {'in': set(), 'out': set()}
        for m in l.outbound_layers:
            if m not in G:
                G[m] = {'in': set(), 'out': set()}
            G[l]['out'].add(m)
            G[m]['in'].add(l)
